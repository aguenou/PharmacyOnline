...
import { pourReserver, retirerMedicament } from '../actions/panierActions';
...
function ReservationScreen(props) {
  ...
  const confirmerReservation = ()=>{
    panierItems.map(item=>{
        ...
        dispatch(pourReserver(qte,prix,id,user,nombreDisponible));
        alert('Réservation effectuée avec succes');
        console.log(qte);
    })
    ...
    return <div className="panier">
        <div className="panier-list">
            <ul className="panier-list-container">
                <li>
                    <h3>
                        <b>Veuillez confirmer la réservation de: </b>
                    </h3>
                    <div>
                        Prix
                    </div>
        ...
        <div className="panier-action">
            ...
            <button onClick={confirmerReservation} className="button primary full-width" disabled={panierItems.length === 0}>
